...
const PEDAGOGICO_ITEMS: MenuItem[] = [
...
const UNIDADE_ITEMS: MenuItem[] = [
  { path: '/app/unidade', label: 'Gestão da Unidade', icon: <Package className="h-4 w-4" /> },
  { path: '/app/plannings', label: 'Planejamentos', icon: <BookOpen className="h-4 w-4" /> },
  { path: '/app/diary', label: 'Diário', icon: <ClipboardList className="h-4 w-4" /> },
...
const PROFESSOR_TOOLS: MenuItem[] = [
  { path: '/app/plannings', label: 'Planejamentos', icon: <BookOpen className="h-4 w-4" /> },
  { path: '/app/diary', label: 'Diário', icon: <ClipboardList className="h-4 w-4" /> },
  { path: '/app/reports', label: 'Relatórios', icon: <BarChart2 className="h-4 w-4" /> },
];
  ...
  return (
    <aside className="w-64 bg-gray-900 text-white min-h-screen flex flex-col">
      ...
      <nav className="flex-1 p-3 space-y-5 overflow-y-auto">
        {isDeveloper && (
          <>
            <NavSection titulo="Pedagógico" items={PEDAGOGICO_ITEMS} isActive={isActive} />
            <NavSection titulo="Unidade" items={UNIDADE_ITEMS} isActive={isActive} />
        ...
        {!isDeveloper && isUnidade && (
          <>
            <NavSection titulo="Gestão" items={UNIDADE_ITEMS} isActive={isActive} />
            <NavSection titulo="Pedagógico" items={PEDAGOGICO_ITEMS} isActive={isActive} />
          </>
        )}
        {!isDeveloper && !isUnidade && isProfessor && (
          <>
            <NavSection titulo="Pedagógico" items={PEDAGOGICO_ITEMS} isActive={isActive} />
            <NavSection titulo="Ferramentas" items={PROFESSOR_TOOLS} isActive={isActive} />
          </>
        )}
        {!isDeveloper && !isMantenedora && !isCentral && !isUnidade && !isProfessor && (
          <NavSection titulo="Menu" items={UNIDADE_ITEMS} isActive={isActive} />
        )}
      </nav>
